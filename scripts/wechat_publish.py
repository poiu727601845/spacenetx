...
SITE_ROOT  = Path("/home/ubuntu/wechat-publisher/site")
DATA_DIR   = Path("/home/ubuntu/wechat-publisher/data")
...
def get_current_ip():
    ...
    try:
        req = urllib.request.Request(
            ...
            headers={"User-Agent": "Mozilla/5.0 (compatible; spacenetx)"},
        )
        with urllib.request.urlopen(req, timeout=10) as resp:
            data = json.loads(resp.read().decode("utf-8"))
            _CURRENT_IP = data.get("ip")
            return _CURRENT_IP
    except Exception:
        return None
...
def _ip_hint():
    ...
    ip = get_current_ip()
    if ip:
        return f"当前公网 IP: {ip}，请确认微信后台「IP白名单」是否包含此 IP。"
    return "请检查微信后台「IP白名单」是否与当前公网 IP一致。"
...
def get_access_token():
    ...
    if "errcode" in resp and resp["errcode"] != 0:
        ...
        if errcode in (401, 40164):
            print(f"  [FAIL] access_token 失败（IP白名单问题）\n"
...
def clean_html(raw_html):
    ...
    # 9b. 移除相对路径 <a> 标签（微信 draft/add API 返回 45166 拒绝 href="/" 这类相对链接）
    text = re.sub(r'(?s)<a[^>]*href=["\']/([^"]*?)["\'][^>]*>.*?</a>', '', text)
...
def make_preview_card(tags):
    """生成「下期预告 + 关注引导」卡片（根据文章标签动态生成）"""
    # 根据当前文章的标签，选一个「下一期」方向
    topic_map = {
        "电网": "电网ETF（562550）最新政策催化与估值分析",
        "半导体": "半导体ETF（512480）Q3订单数据与设备国产化进展",
        "AI": "AI主题基金持仓调整——百模大战后的选股逻辑",
        "ETF": "市场风格切换信号——大盘 vs 小盘ETF怎么配",
        "资金流向": "北向资金连续加仓板块深度复盘",
        "复盘": "本周ETF涨跌幅排行 + 主力资金路线图",
        "新能源": "光伏ETF vs 新能源车ETF——谁先触底反弹",
        "消费": "消费板块底部信号——食品饮料ETF性价比分析",
    }
    # 找匹配的标签
    preview_topic = None
    for tag in tags:
    ...
    if not preview_topic:
        preview_topic = "继续跟踪资金面信号——每日量化复盘，捕捉最佳入场时机"
    ...
    return (
        ...
        f'下一期将聚焦：<strong style="color:#f59e0b;">{preview_topic}</strong></p>'
...
def extract_content(html_path, tags=None):
    ...
    preview_card = make_preview_card(tags)
...
def push_draft(article_path=None):
    ...
    tags = latest.get("tags", []) if not article_path else []
    body = extract_content(article_file, tags)
...
def main():
    # 打印当前公网 IP（方便排查微信 IP 白名单问题）
    ip = get_current_ip()
    if ip:
        print(f"[I] 当前公网 IP: {ip}")
